...
`starter/` enthält Spring Boot, das vollständige Schema und die Seed-Daten aus B2, Response-DTO, Fehlerkörper und ein fertiges Read-Repository für `authors`. Offen sind nur die Methoden der `Book`-Hauptressource.
...
Implementiert `findAll` und `findById` im `JdbcBookRepository`. GET liefert `BookResponse`, nie direkt die Domain-Entity. Unbekannte IDs ergeben 404.
...
Die Anwendung startet. GET-Liste und GET nach ID liefern die B2-Daten über das Response-DTO. ID 1 enthält `9783446279899`.
