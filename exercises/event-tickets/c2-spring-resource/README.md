...
`Venue` ist der Domänen-Record. `VenueJpaEntity` bildet die Tabelle für JPA ab. `SpringDataVenueRepository` und die Infrastruktur sind vorbereitet. Der Controller, der Service und `JpaVenueRepository` enthalten die TODOs für beide GET-Wege. `JpaLookupRepository` liest `organizers`.
...
1. Implementiert `VenueResponse.from(Venue)`.
2. Implementiert `findAll` in `JpaVenueRepository`. Nutzt das Spring-Data-Repository, sortiert nach ID und mappt jede `VenueJpaEntity` auf `Venue`.
3. Implementiert `findAll` im Service und danach im Controller. Der Controller mappt Domänenobjekte auf `VenueResponse`.
...
Beide GET-Endpunkte lesen über Service, Repository-Interface und JPA-Adapter. Der Controller gibt `VenueResponse` zurück. Eine unbekannte ID ergibt 404. Die aktivierten Repository- und API-Checkpoints laufen.
