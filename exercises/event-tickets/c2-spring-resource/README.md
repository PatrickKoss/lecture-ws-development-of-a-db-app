...
Die fachliche Klasse `Venue` bleibt ein unveränderlicher Java-Record. `VenueJpaEntity` bildet die Tabelle für JPA ab. `SpringDataVenueRepository` erweitert `JpaRepository` und ist fertig. Offen sind `findAll` und `findById` im Adapter `JpaVenueRepository`. Das vorbereitete `JpaLookupRepository` liest `organizers`.
...
1. Implementiert `findAll` in `JpaVenueRepository`. Nutzt das Spring-Data-Repository und mappt jede `VenueJpaEntity` auf `Venue`.
...
4. Startet die Anwendung und prüft GET-Liste sowie GET nach ID über Swagger UI. Der Controller liefert `VenueResponse`, keine JPA-Entity. Eine unbekannte ID ergibt 404.
...
Die Anwendung startet. Beide GET-Endpunkte liefern die Seed-Daten über das Response-DTO. ID 1 enthält `V-01`. `JpaRepositoryExerciseTest` und `OpenApiStarterTest` laufen.
...
Welche Aufgabe hat `VenueJpaEntity`? Warum gibt der Controller trotzdem `VenueResponse` zurück? Wo endet Spring Data und wo beginnt euer Adapter?
