...
- venueCode: Text, 2 bis 12 Zeichen, nicht leer
- name: Text, 1 bis 120 Zeichen, nicht leer
- street: Text, 1 bis 120 Zeichen, nicht leer
- postalCode: genau fünf Ziffern
- city: Text, 1 bis 80 Zeichen, nicht leer
- capacity: Ganzzahl von 1 bis 50000
...
1. Ergänzt die Validierungsannotationen in `CreateVenueRequest` und implementiert `toCommand()`. Die ID gehört nicht in den Request.
2. Implementiert `existsByVenueCode(String)` und `save(Venue)` in `JpaVenueRepository`. Nutzt die vorbereitete Spring-Data-Methode. Mappt beim Speichern auf `VenueJpaEntity`, verwendet `saveAndFlush` und reicht Datenbankfehler an `SQLiteConstraintTranslator` weiter.
3. Implementiert `create` im Service. Prüft `venueCode` vor dem Speichern und werft bei einem Duplikat die vorbereitete Konfliktausnahme.
4. Implementiert POST im Controller. Validiert den Request, ruft den Service auf und liefert 201 mit dem gespeicherten `VenueResponse` sowie einem `Location`-Header auf die neue Ressource.
...
2. Implementiert und aktiviert in `CreateApiExampleTest` den TODO-Test `rejectsDuplicateBusinessKey` für ein doppeltes `venueCode`. Legt den Konflikt über zwei Requests oder mit den Seed-Daten an. Prüft Status 409 und den Fehlerkörper.
...
GET und POST laufen durch alle Schichten. POST liefert 201 mit `Location`. Validation liefert 400, ein doppeltes `venueCode` liefert 409 und eine unbekannte ID 404. Die beiden bereitgestellten Beispiele und eure beiden Tests laufen.
...
Warum reicht die vorherige `existsByVenueCode`-Abfrage nicht als alleiniger Schutz vor Duplikaten? Welche Fehlerdetails braucht ein Client, und welche gehören nur ins Server-Log?
