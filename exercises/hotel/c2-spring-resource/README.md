...
`starter/` enthält Spring Boot, das vollständige Schema und die Seed-Daten aus B2, Response-DTO, Fehlerkörper und ein fertiges Read-Repository für `services`. Offen sind nur die Methoden der `RoomType`-Hauptressource.
...
Implementiert `findAll` und `findById` im `JdbcRoomTypeRepository`. GET liefert `RoomTypeResponse`, nie direkt die Domain-Entity. Unbekannte IDs ergeben 404.
...
Die Anwendung startet. GET-Liste und GET nach ID liefern die B2-Daten über das Response-DTO. ID 1 enthält `EZ`.
