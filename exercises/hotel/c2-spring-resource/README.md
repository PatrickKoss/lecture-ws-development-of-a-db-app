...
`RoomType` ist der Domänen-Record. `RoomTypeJpaEntity` bildet die Tabelle für JPA ab. `SpringDataRoomTypeRepository` und die Infrastruktur sind vorbereitet. Der Controller, der Service und `JpaRoomTypeRepository` enthalten die TODOs für beide GET-Wege. `JpaLookupRepository` liest `services`.
...
1. Implementiert `RoomTypeResponse.from(RoomType)`.
2. Implementiert `findAll` in `JpaRoomTypeRepository`. Nutzt das Spring-Data-Repository, sortiert nach ID und mappt jede `RoomTypeJpaEntity` auf `RoomType`.
3. Implementiert `findAll` im Service und danach im Controller. Der Controller mappt Domänenobjekte auf `RoomTypeResponse`.
...
Beide GET-Endpunkte lesen über Service, Repository-Interface und JPA-Adapter. Der Controller gibt `RoomTypeResponse` zurück. Eine unbekannte ID ergibt 404. Die aktivierten Repository- und API-Checkpoints laufen.
