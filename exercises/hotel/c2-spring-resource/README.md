...
Die fachliche Klasse `RoomType` bleibt ein unveränderlicher Java-Record. `RoomTypeJpaEntity` bildet die Tabelle für JPA ab. `SpringDataRoomTypeRepository` erweitert `JpaRepository` und ist fertig. Offen sind `findAll` und `findById` im Adapter `JpaRoomTypeRepository`. Das vorbereitete `JpaLookupRepository` liest `services`.
...
1. Implementiert `findAll` in `JpaRoomTypeRepository`. Nutzt das Spring-Data-Repository und mappt jede `RoomTypeJpaEntity` auf `RoomType`.
...
4. Startet die Anwendung und prüft GET-Liste sowie GET nach ID über Swagger UI. Der Controller liefert `RoomTypeResponse`, keine JPA-Entity. Eine unbekannte ID ergibt 404.
...
Die Anwendung startet. Beide GET-Endpunkte liefern die Seed-Daten über das Response-DTO. ID 1 enthält `EZ`. `JpaRepositoryExerciseTest` und `OpenApiStarterTest` laufen.
...
Welche Aufgabe hat `RoomTypeJpaEntity`? Warum gibt der Controller trotzdem `RoomTypeResponse` zurück? Wo endet Spring Data und wo beginnt euer Adapter?
