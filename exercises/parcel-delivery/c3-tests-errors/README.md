...
- trackingCode: Text, 1 bis 40 Zeichen, nicht leer
- recipient: Text, 1 bis 160 Zeichen, nicht leer
- weight: Dezimalzahl von 0 bis 31,5
...
1. Ergänzt die Validierungsannotationen in `CreateParcelRequest` und implementiert `toCommand()`. Die ID gehört nicht in den Request.
2. Implementiert `existsByTrackingCode(String)` und `save(Parcel)` in `JpaParcelRepository`. Nutzt die vorbereitete Spring-Data-Methode. Mappt beim Speichern auf `ParcelJpaEntity`, verwendet `saveAndFlush` und reicht Datenbankfehler an `SQLiteConstraintTranslator` weiter.
3. Implementiert `create` im Service. Prüft `trackingCode` vor dem Speichern und werft bei einem Duplikat die vorbereitete Konfliktausnahme.
4. Implementiert POST im Controller. Validiert den Request, ruft den Service auf und liefert 201 mit dem gespeicherten `ParcelResponse` sowie einem `Location`-Header auf die neue Ressource.
...
2. Implementiert und aktiviert in `CreateApiExampleTest` den TODO-Test `rejectsDuplicateBusinessKey` für ein doppeltes `trackingCode`. Legt den Konflikt über zwei Requests oder mit den Seed-Daten an. Prüft Status 409 und den Fehlerkörper.
...
GET und POST laufen durch alle Schichten. POST liefert 201 mit `Location`. Validation liefert 400, ein doppeltes `trackingCode` liefert 409 und eine unbekannte ID 404. Die beiden bereitgestellten Beispiele und eure beiden Tests laufen.
...
Warum reicht die vorherige `existsByTrackingCode`-Abfrage nicht als alleiniger Schutz vor Duplikaten? Welche Fehlerdetails braucht ein Client, und welche gehören nur ins Server-Log?
