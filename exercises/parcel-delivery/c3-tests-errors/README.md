...
Implementiert `insert`, `existsByTrackingCode` und prüft die vorhandene POST-Methode. Aktiviert den Controller-Test. Prüft 201, einen Validierungsfehler mit 400 und den doppelten `tracking_code` mit 409. Der Fehlerkörper enthält `code`, `message`, `correlationId` und bei Validierung `fields`.
