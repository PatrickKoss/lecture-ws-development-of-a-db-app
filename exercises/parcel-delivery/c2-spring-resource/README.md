...
`starter/` enthält Spring Boot, das vollständige Schema und die Seed-Daten aus B2, Response-DTO, Fehlerkörper und ein fertiges Read-Repository für `depots`. Offen sind nur die Methoden für `Parcel`.
...
Implementiert `findAll` und `findById` im `JdbcParcelRepository`. GET liefert `ParcelResponse`, nie direkt die Domain-Entity. Unbekannte IDs ergeben 404.
...
Die Anwendung startet. GET-Liste und GET nach ID liefern die B2-Daten über das Response-DTO. ID 1 enthält `PK-01`.
