...
`Parcel` ist der Domänen-Record. `ParcelJpaEntity` bildet die Tabelle für JPA ab. `SpringDataParcelRepository` und die Infrastruktur sind vorbereitet. Der Controller, der Service und `JpaParcelRepository` enthalten die TODOs für beide GET-Wege. `JpaLookupRepository` liest `depots`.
...
1. Implementiert `ParcelResponse.from(Parcel)`.
2. Implementiert `findAll` in `JpaParcelRepository`. Nutzt das Spring-Data-Repository, sortiert nach ID und mappt jede `ParcelJpaEntity` auf `Parcel`.
3. Implementiert `findAll` im Service und danach im Controller. Der Controller mappt Domänenobjekte auf `ParcelResponse`.
...
Beide GET-Endpunkte lesen über Service, Repository-Interface und JPA-Adapter. Der Controller gibt `ParcelResponse` zurück. Eine unbekannte ID ergibt 404. Die aktivierten Repository- und API-Checkpoints laufen.
