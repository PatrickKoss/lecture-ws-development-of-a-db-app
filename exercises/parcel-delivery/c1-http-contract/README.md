...
Arbeitet im Projekt `../c2-spring-resource/starter`. Das technische Spring-Gerüst kompiliert bereits. `CreateParcelRequest` und `ParcelResponse` sind absichtlich leere Records. Der Controller enthält die Verdrahtung und die Routen für GET und POST. Die dokumentierte Listenroute dient als Beispiel. Bei GET nach ID und POST ergänzt ihr den Antwortvertrag selbst. Die Implementierungen folgen in C2 und C3.
...
`ParcelResponse` enthält diese Komponenten in dieser Reihenfolge:

```text
Long id, String trackingCode, String recipient, BigDecimal weight
```

`CreateParcelRequest` enthält dieselben fachlichen Komponenten ohne `Long id`. Die ID vergibt der Server. Verwendet die angegebenen Java-Typen und Feldnamen unverändert, damit die späteren Checkpoints anschließen können. Validierungsannotationen und Konvertierung ergänzt ihr in C3.

## Kernauftrag

1. Exportiert die unveränderte Spec als `openapi.before.json`.
2. Ergänzt die Komponenten von `CreateParcelRequest` und `ParcelResponse`. Beide Records müssen weiter kompilieren. Lasst die vorbereiteten TODO-Methoden zunächst offen.
...
4. Vervollständigt den OpenAPI-Vertrag im Controller. Die Liste liefert 200 mit einem Array aus `ParcelResponse`. GET nach ID liefert 200 oder 404 mit `ApiError`. POST liefert 201 mit `Location` sowie 400 und 409. Nennt im Konfliktfall den Schlüssel `trackingCode`.
