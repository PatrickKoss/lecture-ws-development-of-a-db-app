...
class OpenApiTest {
  ...
  void generatesContractFromControllerAndModels() throws Exception {
    mvc.perform(get("/v3/api-docs"))
        ...
        .andExpect(jsonPath("$.paths['/api/movies'].post.requestBody.content['application/json'].schema['$ref']")
            .value("#/components/schemas/CreateMovieRequest"))
        .andExpect(jsonPath("$.paths['/api/movies'].post.responses['201'].content['application/json'].schema['$ref']")
            .value("#/components/schemas/MovieResponse"))
        .andExpect(jsonPath("$.paths['/api/movies'].post.responses['201'].headers.Location").exists())
        .andExpect(jsonPath("$.paths['/api/movies'].post.responses['400'].content['application/json'].schema['$ref']")
  ...
  void createsResponseWithLocation() throws Exception {
    var value = mock(Movie.class);
    when(value.id()).thenReturn(42L);
    when(service.create(any(CreateMovieRequest.class))).thenReturn(value);
    mvc.perform(post("/api/movies").contentType("application/json")
            .content("{\"movieCode\": \"F-199\", \"title\": \"Kursfilm\", \"releaseYear\": 2026, \"durationMinutes\": 90, \"fskCode\": \"FSK_12\", \"minimumAge\": 12}"))
        .andExpect(status().isCreated())
        .andExpect(header().string("Location", "http://localhost/api/movies/42"))
