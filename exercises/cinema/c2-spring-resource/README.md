...
`Movie` ist der Domänen-Record. `MovieJpaEntity` bildet die Tabelle für JPA ab. `SpringDataMovieRepository` und die Infrastruktur sind vorbereitet. Der Controller, der Service und `JpaMovieRepository` enthalten die TODOs für beide GET-Wege. `JpaLookupRepository` liest `halls`.
...
1. Implementiert `MovieResponse.from(Movie)`.
2. Implementiert `findAll` in `JpaMovieRepository`. Nutzt das Spring-Data-Repository, sortiert nach ID und mappt jede `MovieJpaEntity` auf `Movie`.
3. Implementiert `findAll` im Service und danach im Controller. Der Controller mappt Domänenobjekte auf `MovieResponse`.
...
Beide GET-Endpunkte lesen über Service, Repository-Interface und JPA-Adapter. Der Controller gibt `MovieResponse` zurück. Eine unbekannte ID ergibt 404. Die aktivierten Repository- und API-Checkpoints laufen.
