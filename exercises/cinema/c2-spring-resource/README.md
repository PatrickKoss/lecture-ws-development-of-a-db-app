...
Die fachliche Klasse `Movie` bleibt ein unveränderlicher Java-Record. `MovieJpaEntity` bildet die Tabelle für JPA ab. `SpringDataMovieRepository` erweitert `JpaRepository` und ist fertig. Offen sind `findAll` und `findById` im Adapter `JpaMovieRepository`. Das vorbereitete `JpaLookupRepository` liest `halls`.
...
1. Implementiert `findAll` in `JpaMovieRepository`. Nutzt das Spring-Data-Repository und mappt jede `MovieJpaEntity` auf `Movie`.
...
4. Startet die Anwendung und prüft GET-Liste sowie GET nach ID über Swagger UI. Der Controller liefert `MovieResponse`, keine JPA-Entity. Eine unbekannte ID ergibt 404.
...
Die Anwendung startet. Beide GET-Endpunkte liefern die Seed-Daten über das Response-DTO. ID 1 enthält `F-101`. `JpaRepositoryExerciseTest` und `OpenApiStarterTest` laufen.
...
Welche Aufgabe hat `MovieJpaEntity`? Warum gibt der Controller trotzdem `MovieResponse` zurück? Wo endet Spring Data und wo beginnt euer Adapter?
