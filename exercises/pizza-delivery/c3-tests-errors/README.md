...
- pizzaNumber: Text im Format P-99
- name: Text, 1 bis 100 Zeichen, nicht leer
- category: Text, 1 bis 30 Zeichen, nicht leer
- ovenStation: Text, 1 bis 20 Zeichen, nicht leer
- basePrice: positive Dezimalzahl
...
1. Ergänzt die Validierungsannotationen in `CreatePizzaRequest` und implementiert `toCommand()`. Die ID gehört nicht in den Request.
2. Implementiert `existsByPizzaNumber(String)` und `save(Pizza)` in `JpaPizzaRepository`. Nutzt die vorbereitete Spring-Data-Methode. Mappt beim Speichern auf `PizzaJpaEntity`, verwendet `saveAndFlush` und reicht Datenbankfehler an `SQLiteConstraintTranslator` weiter.
3. Implementiert `create` im Service. Prüft `pizzaNumber` vor dem Speichern und werft bei einem Duplikat die vorbereitete Konfliktausnahme.
4. Implementiert POST im Controller. Validiert den Request, ruft den Service auf und liefert 201 mit dem gespeicherten `PizzaResponse` sowie einem `Location`-Header auf die neue Ressource.
...
2. Implementiert und aktiviert in `CreateApiExampleTest` den TODO-Test `rejectsDuplicateBusinessKey` für ein doppeltes `pizzaNumber`. Legt den Konflikt über zwei Requests oder mit den Seed-Daten an. Prüft Status 409 und den Fehlerkörper.
...
GET und POST laufen durch alle Schichten. POST liefert 201 mit `Location`. Validation liefert 400, ein doppeltes `pizzaNumber` liefert 409 und eine unbekannte ID 404. Die beiden bereitgestellten Beispiele und eure beiden Tests laufen.
...
Warum reicht die vorherige `existsByPizzaNumber`-Abfrage nicht als alleiniger Schutz vor Duplikaten? Welche Fehlerdetails braucht ein Client, und welche gehören nur ins Server-Log?
