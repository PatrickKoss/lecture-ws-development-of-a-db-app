...
`Pizza` ist der Domänen-Record. `PizzaJpaEntity` bildet die Tabelle für JPA ab. `SpringDataPizzaRepository` und die Infrastruktur sind vorbereitet. Der Controller, der Service und `JpaPizzaRepository` enthalten die TODOs für beide GET-Wege. `JpaLookupRepository` liest `toppings`.
...
1. Implementiert `PizzaResponse.from(Pizza)`.
2. Implementiert `findAll` in `JpaPizzaRepository`. Nutzt das Spring-Data-Repository, sortiert nach ID und mappt jede `PizzaJpaEntity` auf `Pizza`.
3. Implementiert `findAll` im Service und danach im Controller. Der Controller mappt Domänenobjekte auf `PizzaResponse`.
...
Beide GET-Endpunkte lesen über Service, Repository-Interface und JPA-Adapter. Der Controller gibt `PizzaResponse` zurück. Eine unbekannte ID ergibt 404. Die aktivierten Repository- und API-Checkpoints laufen.
