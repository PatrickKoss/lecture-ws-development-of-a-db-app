...
Arbeitet mit eurem lauffähigen Stand aus `../b3-jdbc/starter`. `starter/PizzaCatalog.java` ist die vorbereitete aufrufende Schicht. Ihr kopiert sie in das B3-Projekt.
...
1. Entfernt `SQLException` aus der öffentlichen Schnittstelle `PizzaRepository`.
2. Fangt `SQLException` ausschließlich in `JdbcPizzaRepository` ab und übersetzt sie in eine eigene ungeprüfte `RepositoryException`.
...
4. Lasst `PizzaCatalog` nur über `PizzaRepository` auf Pizza-Daten zugreifen. Im Catalog stehen weder SQL noch JDBC-Typen.
5. Führt die vorhandenen Tests aus. Der JDBC-Test muss weiter dieselben Zeilen lesen.

## Vertiefung

Implementiert `InMemoryPizzaRepository`. Es vergibt IDs und lehnt einen doppelten fachlichen Schlüssel genauso ab wie die UNIQUE-Regel der Datenbank. Testet `PizzaCatalog` mit diesem Repository, ohne SQLite zu öffnen.

## Vorbereiteter Zwischenstand

Wenn das Refactoring stockt, legt zuerst die endgültigen Methodensignaturen von `PizzaRepository` fest. Verschiebt danach eine Methode vollständig in `JdbcPizzaRepository` und übersetzt ihren SQL-Fehler an der Repository-Grenze.
