...
- stationCode: Text, 3 bis 8 Zeichen, nicht leer
- name: Text, 1 bis 80 Zeichen, nicht leer
- address: Text, 1 bis 120 Zeichen, nicht leer
- capacity: Ganzzahl von 5 bis 100
- status: ACTIVE, PLANNED oder CLOSED
...
1. Ergänzt die Validierungsannotationen in `CreateStationRequest` und implementiert `toCommand()`. Die ID gehört nicht in den Request.
2. Implementiert `existsByStationCode(String)` und `save(Station)` in `JpaStationRepository`. Nutzt die vorbereitete Spring-Data-Methode. Mappt beim Speichern auf `StationJpaEntity`, verwendet `saveAndFlush` und reicht Datenbankfehler an `SQLiteConstraintTranslator` weiter.
3. Implementiert `create` im Service. Prüft `stationCode` vor dem Speichern und werft bei einem Duplikat die vorbereitete Konfliktausnahme.
4. Implementiert POST im Controller. Validiert den Request, ruft den Service auf und liefert 201 mit dem gespeicherten `StationResponse` sowie einem `Location`-Header auf die neue Ressource.
...
2. Implementiert und aktiviert in `CreateApiExampleTest` den TODO-Test `rejectsDuplicateBusinessKey` für ein doppeltes `stationCode`. Legt den Konflikt über zwei Requests oder mit den Seed-Daten an. Prüft Status 409 und den Fehlerkörper.
...
GET und POST laufen durch alle Schichten. POST liefert 201 mit `Location`. Validation liefert 400, ein doppeltes `stationCode` liefert 409 und eine unbekannte ID 404. Die beiden bereitgestellten Beispiele und eure beiden Tests laufen.
...
Warum reicht die vorherige `existsByStationCode`-Abfrage nicht als alleiniger Schutz vor Duplikaten? Welche Fehlerdetails braucht ein Client, und welche gehören nur ins Server-Log?
