...
Implementiert `insert`, `existsByStationCode` und POST. Aktiviert den Controller-Test. Prüft 201, einen Validierungsfehler mit 400 und den doppelten `station_code` mit 409. Der Fehlerkörper enthält `code`, `message`, `correlationId` und bei Validierung `fields`.
