...
Die fachliche Klasse `Station` bleibt ein unveränderlicher Java-Record. `StationJpaEntity` bildet die Tabelle für JPA ab. `SpringDataStationRepository` erweitert `JpaRepository` und ist fertig. Offen sind `findAll` und `findById` im Adapter `JpaStationRepository`. Das vorbereitete `JpaLookupRepository` liest `bike_models`.
...
1. Implementiert `findAll` in `JpaStationRepository`. Nutzt das Spring-Data-Repository und mappt jede `StationJpaEntity` auf `Station`.
...
4. Startet die Anwendung und prüft GET-Liste sowie GET nach ID über Swagger UI. Der Controller liefert `StationResponse`, keine JPA-Entity. Eine unbekannte ID ergibt 404.
...
Die Anwendung startet. Beide GET-Endpunkte liefern die Seed-Daten über das Response-DTO. ID 1 enthält `DOM`. `JpaRepositoryExerciseTest` und `OpenApiStarterTest` laufen.
...
Welche Aufgabe hat `StationJpaEntity`? Warum gibt der Controller trotzdem `StationResponse` zurück? Wo endet Spring Data und wo beginnt euer Adapter?
