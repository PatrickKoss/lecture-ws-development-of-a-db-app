...
`Station` ist der Domänen-Record. `StationJpaEntity` bildet die Tabelle für JPA ab. `SpringDataStationRepository` und die Infrastruktur sind vorbereitet. Der Controller, der Service und `JpaStationRepository` enthalten die TODOs für beide GET-Wege. `JpaLookupRepository` liest `bike_models`.
...
1. Implementiert `StationResponse.from(Station)`.
2. Implementiert `findAll` in `JpaStationRepository`. Nutzt das Spring-Data-Repository, sortiert nach ID und mappt jede `StationJpaEntity` auf `Station`.
3. Implementiert `findAll` im Service und danach im Controller. Der Controller mappt Domänenobjekte auf `StationResponse`.
...
Beide GET-Endpunkte lesen über Service, Repository-Interface und JPA-Adapter. Der Controller gibt `StationResponse` zurück. Eine unbekannte ID ergibt 404. Die aktivierten Repository- und API-Checkpoints laufen.
