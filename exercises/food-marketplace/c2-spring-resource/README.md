...
Die fachliche Klasse `Restaurant` bleibt ein unveränderlicher Java-Record. `RestaurantJpaEntity` bildet die Tabelle für JPA ab. `SpringDataRestaurantRepository` erweitert `JpaRepository` und ist fertig. Offen sind `findAll` und `findById` im Adapter `JpaRestaurantRepository`. Das vorbereitete `JpaLookupRepository` liest `couriers`.
...
1. Implementiert `findAll` in `JpaRestaurantRepository`. Nutzt das Spring-Data-Repository und mappt jede `RestaurantJpaEntity` auf `Restaurant`.
...
4. Startet die Anwendung und prüft GET-Liste sowie GET nach ID über Swagger UI. Der Controller liefert `RestaurantResponse`, keine JPA-Entity. Eine unbekannte ID ergibt 404.
...
Die Anwendung startet. Beide GET-Endpunkte liefern die Seed-Daten über das Response-DTO. ID 1 enthält `R-101`. `JpaRepositoryExerciseTest` und `OpenApiStarterTest` laufen.
...
Welche Aufgabe hat `RestaurantJpaEntity`? Warum gibt der Controller trotzdem `RestaurantResponse` zurück? Wo endet Spring Data und wo beginnt euer Adapter?
