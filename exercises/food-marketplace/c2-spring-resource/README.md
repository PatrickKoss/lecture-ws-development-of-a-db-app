...
`Restaurant` ist der Domänen-Record. `RestaurantJpaEntity` bildet die Tabelle für JPA ab. `SpringDataRestaurantRepository` und die Infrastruktur sind vorbereitet. Der Controller, der Service und `JpaRestaurantRepository` enthalten die TODOs für beide GET-Wege. `JpaLookupRepository` liest `couriers`.
...
1. Implementiert `RestaurantResponse.from(Restaurant)`.
2. Implementiert `findAll` in `JpaRestaurantRepository`. Nutzt das Spring-Data-Repository, sortiert nach ID und mappt jede `RestaurantJpaEntity` auf `Restaurant`.
3. Implementiert `findAll` im Service und danach im Controller. Der Controller mappt Domänenobjekte auf `RestaurantResponse`.
...
Beide GET-Endpunkte lesen über Service, Repository-Interface und JPA-Adapter. Der Controller gibt `RestaurantResponse` zurück. Eine unbekannte ID ergibt 404. Die aktivierten Repository- und API-Checkpoints laufen.
