...
Nutzt das Modell `Exhibit` aus B3 und das Projekt `../c2-spring-resource/starter`.
SpringDoc ist eingebunden. `ExhibitController` enthält ein dokumentiertes POST-Beispiel mit `CreateExhibitRequest`, `ExhibitResponse`, 201, `Location`, 400 und 409.
...
2. Ergänzt Beschreibungen und passende Beispiele an den Feldern von `CreateExhibitRequest` und `ExhibitResponse` mit `@Schema`. Beachtet die vorhandenen Validierungsregeln. Der Request enthält keine Server-ID.
3. Dokumentiert beide GET-Methoden mit `@Operation` und `@ApiResponse`. Die Liste liefert 200 mit einem Array aus `ExhibitResponse`, GET nach ID liefert 200 oder 404 mit `ApiError`. Für das Array nutzt ihr `@Content(array = @ArraySchema(schema = @Schema(implementation = ExhibitResponse.class)))` und importiert `io.swagger.v3.oas.annotations.media.ArraySchema`.
4. Beschreibt den 409-Fachkonflikt konkret: `inventory_code` darf nicht doppelt vorkommen. Startet nach Codeänderungen neu. Prüft in Swagger UI Request, Response, Pflichtfelder und Statuscodes.
