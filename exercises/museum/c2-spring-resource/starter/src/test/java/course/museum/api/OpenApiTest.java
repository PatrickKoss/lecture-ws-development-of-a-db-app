...
class OpenApiTest {
  ...
  void generatesContractFromControllerAndModels() throws Exception {
    mvc.perform(get("/v3/api-docs"))
        ...
        .andExpect(jsonPath("$.paths['/api/exhibits/{id}'].get").exists())
        .andExpect(jsonPath("$.paths['/api/exhibits'].post.requestBody.content['application/json'].schema['$ref']")
            .value("#/components/schemas/CreateExhibitRequest"))
        .andExpect(jsonPath("$.paths['/api/exhibits'].post.responses['201'].content['application/json'].schema['$ref']")
            .value("#/components/schemas/ExhibitResponse"))
        .andExpect(jsonPath("$.paths['/api/exhibits'].post.responses['201'].headers.Location").exists())
        .andExpect(jsonPath("$.paths['/api/exhibits'].post.responses['400'].content['application/json'].schema['$ref']")
  ...
  void createsResponseWithLocation() throws Exception {
    var value = mock(Exhibit.class);
    when(value.id()).thenReturn(42L);
    when(service.create(any(CreateExhibitRequest.class))).thenReturn(value);
    mvc.perform(post("/api/exhibits").contentType("application/json")
            .content("{\"inventoryCode\": \"EX-99\", \"title\": \"Testdatensatz\", \"insuredValue\": 1.0}"))
        .andExpect(status().isCreated())
        .andExpect(header().string("Location", "http://localhost/api/exhibits/42"))
        .andExpect(jsonPath("$.id").value(42));
  }

  @Test
  void validatesBeforeCallingService() throws Exception {
    mvc.perform(post("/api/exhibits").contentType("application/json").content("{}"))
