...
`Exhibit` ist der Domänen-Record. `ExhibitJpaEntity` bildet die Tabelle für JPA ab. `SpringDataExhibitRepository` und die Infrastruktur sind vorbereitet. Der Controller, der Service und `JpaExhibitRepository` enthalten die TODOs für beide GET-Wege. `JpaLookupRepository` liest `galleries`.
...
1. Implementiert `ExhibitResponse.from(Exhibit)`.
2. Implementiert `findAll` in `JpaExhibitRepository`. Nutzt das Spring-Data-Repository, sortiert nach ID und mappt jede `ExhibitJpaEntity` auf `Exhibit`.
3. Implementiert `findAll` im Service und danach im Controller. Der Controller mappt Domänenobjekte auf `ExhibitResponse`.
...
Beide GET-Endpunkte lesen über Service, Repository-Interface und JPA-Adapter. Der Controller gibt `ExhibitResponse` zurück. Eine unbekannte ID ergibt 404. Die aktivierten Repository- und API-Checkpoints laufen.
