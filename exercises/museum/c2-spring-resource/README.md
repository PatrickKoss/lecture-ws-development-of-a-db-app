...
Die fachliche Klasse `Exhibit` bleibt ein unveränderlicher Java-Record. `ExhibitJpaEntity` bildet die Tabelle für JPA ab. `SpringDataExhibitRepository` erweitert `JpaRepository` und ist fertig. Offen sind `findAll` und `findById` im Adapter `JpaExhibitRepository`. Das vorbereitete `JpaLookupRepository` liest `galleries`.
...
1. Implementiert `findAll` in `JpaExhibitRepository`. Nutzt das Spring-Data-Repository und mappt jede `ExhibitJpaEntity` auf `Exhibit`.
...
4. Startet die Anwendung und prüft GET-Liste sowie GET nach ID über Swagger UI. Der Controller liefert `ExhibitResponse`, keine JPA-Entity. Eine unbekannte ID ergibt 404.
...
Die Anwendung startet. Beide GET-Endpunkte liefern die Seed-Daten über das Response-DTO. ID 1 enthält `EX-01`. `JpaRepositoryExerciseTest` und `OpenApiStarterTest` laufen.
...
Welche Aufgabe hat `ExhibitJpaEntity`? Warum gibt der Controller trotzdem `ExhibitResponse` zurück? Wo endet Spring Data und wo beginnt euer Adapter?
