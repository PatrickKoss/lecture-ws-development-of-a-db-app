...
`starter/` enthält Spring Boot, das vollständige Schema und die Seed-Daten aus B2, Response-DTO, Fehlerkörper und ein fertiges Read-Repository für `galleries`. Offen sind nur die Methoden für `Exhibit`.
...
Implementiert `findAll` und `findById` im `JdbcExhibitRepository`. GET liefert `ExhibitResponse`, nie direkt die Domain-Entity. Unbekannte IDs ergeben 404.
...
Die Anwendung startet. GET-Liste und GET nach ID liefern die B2-Daten über das Response-DTO. ID 1 enthält `EX-01`.
