...
- resourceCode: Text, 1 bis 40 Zeichen, nicht leer
- name: Text, 1 bis 120 Zeichen, nicht leer
- measure: Dezimalzahl, mindestens 0
...
1. Ergänzt die Validierungsannotationen in `CreateResourceRequest` und implementiert `toCommand()`. Die ID gehört nicht in den Request.
2. Implementiert `existsByResourceCode(String)` und `save(Resource)` in `JpaResourceRepository`. Nutzt die vorbereitete Spring-Data-Methode. Mappt beim Speichern auf `ResourceJpaEntity`, verwendet `saveAndFlush` und reicht Datenbankfehler an `SQLiteConstraintTranslator` weiter.
3. Implementiert `create` im Service. Prüft `resourceCode` vor dem Speichern und werft bei einem Duplikat die vorbereitete Konfliktausnahme.
4. Implementiert POST im Controller. Validiert den Request, ruft den Service auf und liefert 201 mit dem gespeicherten `ResourceResponse` sowie einem `Location`-Header auf die neue Ressource.
...
2. Implementiert und aktiviert in `CreateApiExampleTest` den TODO-Test `rejectsDuplicateBusinessKey` für ein doppeltes `resourceCode`. Legt den Konflikt über zwei Requests oder mit den Seed-Daten an. Prüft Status 409 und den Fehlerkörper.
...
GET und POST laufen durch alle Schichten. POST liefert 201 mit `Location`. Validation liefert 400, ein doppeltes `resourceCode` liefert 409 und eine unbekannte ID 404. Die beiden bereitgestellten Beispiele und eure beiden Tests laufen.
...
Warum reicht die vorherige `existsByResourceCode`-Abfrage nicht als alleiniger Schutz vor Duplikaten? Welche Fehlerdetails braucht ein Client, und welche gehören nur ins Server-Log?
