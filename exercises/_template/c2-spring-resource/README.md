...
`Resource` ist der Domänen-Record. `ResourceJpaEntity` bildet die Tabelle für JPA ab. `SpringDataResourceRepository` und die Infrastruktur sind vorbereitet. Der Controller, der Service und `JpaResourceRepository` enthalten die TODOs für beide GET-Wege. `JpaLookupRepository` liest `reference_items`.
...
1. Implementiert `ResourceResponse.from(Resource)`.
2. Implementiert `findAll` in `JpaResourceRepository`. Nutzt das Spring-Data-Repository, sortiert nach ID und mappt jede `ResourceJpaEntity` auf `Resource`.
3. Implementiert `findAll` im Service und danach im Controller. Der Controller mappt Domänenobjekte auf `ResourceResponse`.
...
Beide GET-Endpunkte lesen über Service, Repository-Interface und JPA-Adapter. Der Controller gibt `ResourceResponse` zurück. Eine unbekannte ID ergibt 404. Die aktivierten Repository- und API-Checkpoints laufen.
