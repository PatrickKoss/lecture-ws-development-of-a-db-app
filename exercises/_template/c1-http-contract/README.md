...
`ResourceResponse` enthält diese Komponenten in dieser Reihenfolge:

```text
Long id, String resourceCode, String name, BigDecimal measure
```

`CreateResourceRequest` enthält dieselben fachlichen Komponenten ohne `Long id`. Die ID vergibt der Server. Verwendet die angegebenen Java-Typen und Feldnamen unverändert, damit die späteren Checkpoints anschließen können. Validierungsannotationen und Konvertierung ergänzt ihr in C3.

## Kernauftrag

1. Exportiert die unveränderte Spec als `openapi.before.json`.
2. Ergänzt die Komponenten von `CreateResourceRequest` und `ResourceResponse`. Beide Records müssen weiter kompilieren. Lasst die vorbereiteten TODO-Methoden zunächst offen.
...
4. Vervollständigt den OpenAPI-Vertrag im Controller. Die Liste liefert 200 mit einem Array aus `ResourceResponse`. GET nach ID liefert 200 oder 404 mit `ApiError`. POST liefert 201 mit `Location` sowie 400 und 409. Nennt im Konfliktfall den Schlüssel `resourceCode`.
