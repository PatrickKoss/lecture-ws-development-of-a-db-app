...
Arbeitet im Projekt `../c2-spring-resource/starter`. Es enthält das Modell `Resource`, getrennte Request- und Response-DTOs sowie SpringDoc. `ResourceController` zeigt am POST-Endpunkt bereits, wie 201, `Location`, 400 und 409 dokumentiert werden.
...
2. Ändert an mindestens drei Feldern von `CreateResourceRequest` und `ResourceResponse` die vorbereitete Beschreibung oder das Beispiel. Verwendet konkrete Werte aus eurer Domäne. Der Request enthält keine vom Server vergebene ID.
3. Formuliert Summary oder Beschreibung beider GET-Methoden fachlich genauer. Prüft dabei den vorbereiteten Vertrag: Die Liste liefert 200 mit einem Array aus `ResourceResponse`. GET nach ID liefert 200 oder 404 mit `ApiError`. Die Liste verwendet `@ArraySchema`.
4. Formuliert die Konfliktbeschreibung mit eigenen Worten und nennt den Schlüssel `resourceCode` ausdrücklich.
