...
`starter/` ist ein eigenständiges Java-21-Projekt. V1 und V2 enthalten den vollständigen Referenzstand aus B1/B2, dazu `Resource` und ein fertiges Read-Repository für `reference_items`.
...
Implementiert `findById`, `findAll` und das sichtbare Row-Mapping in `JdbcResourceRepository`. Aktiviert danach den deaktivierten Repository-Test.
...
Der Test liest mindestens drei `Hauptressource`-Datensätze. ID 1 enthält den aus B2 bekannten Wert `R-01`. Eine unbekannte ID liefert `Optional.empty()`.
