...
`BookController` dokumentiert die Liste, die Einzelabfrage und POST unter
`/api/books`. Die GET-Antworten verwenden `BookResponse`. POST nimmt
`CreateBookRequest` entgegen und liefert bei Erfolg 201 mit einem `Location`-Header.
Wenn `isbn` bereits vergeben ist, beschreibt der Vertrag eine
409-Antwort mit `ApiError`.

Die `@Schema`-Annotationen an beiden DTOs erklären jedes Feld und enthalten
konkrete Beispiele, darunter `isbn`. SpringDoc übernimmt Java-Typen und
