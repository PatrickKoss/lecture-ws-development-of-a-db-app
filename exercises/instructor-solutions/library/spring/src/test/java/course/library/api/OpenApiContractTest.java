  ...
  void documentsAndExportsTheImplementedContract() throws Exception {
    var result =
        mvc.perform(get("/v3/api-docs"))
            ...
            .andExpect(jsonPath("$.paths['/api/books'].post").exists())
            .andExpect(jsonPath("$.paths['/api/books/{id}'].get").exists())
            .andExpect(jsonPath("$.paths['/api/books/{id}'].put").exists())
            .andExpect(jsonPath("$.paths['/api/books/{id}'].delete").exists())
            .andExpect(
                jsonPath("$.components.schemas.CreateBookRequest.properties.id").doesNotExist())
            .andExpect(
                jsonPath("$.components.schemas.BookResponse.properties.id.readOnly").value(true))
            .andReturn();
