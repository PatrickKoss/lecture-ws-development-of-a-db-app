...
class BookApiIntegrationTest {
  ...
  private static final String VALID =
      "{\"isbn\": \"9780000000000\", \"title\": \"Testbuch\", \"publicationYear\": 2026,"
          + " \"subjectArea\": \"Informatik\", \"shelfCode\": \"I-99\"}";
  ...
  void readsSeedDataAndReportsMissingIds() throws Exception {
    mvc.perform(get(BASE))
        ...
        .andExpect(jsonPath("$[0].isbn").value("9783446279899"));
  ...
  void createsReplacesAndDeletesThroughRealPersistence() throws Exception {
    ...
    mvc.perform(
            put(BASE + "/" + id)
                ...
                .content(
                    "{\"isbn\": \"9780000000000\", \"title\": \"Testbuch aktualisiert\","
                        + " \"publicationYear\": 2026, \"subjectArea\": \"Informatik\","
                        + " \"shelfCode\": \"I-99\"}"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.id").value(id))
        .andExpect(jsonPath("$.title").value("Testbuch aktualisiert"));
    ...
    mvc.perform(
            put(BASE + "/" + id)
                .contentType(MediaType.APPLICATION_JSON)
                .content(
                    "{\"isbn\": \"9783446279899\", \"title\": \"Testbuch aktualisiert\","
                        + " \"publicationYear\": 2026, \"subjectArea\": \"Informatik\","
                        + " \"shelfCode\": \"I-99\"}"))
        .andExpect(status().isConflict())
        .andExpect(jsonPath("$.code").value("ISBN_EXISTS"));
    mvc.perform(get(BASE + "/" + id))
        .andExpect(jsonPath("$.isbn").value("9780000000000"))
        .andExpect(jsonPath("$.title").value("Testbuch aktualisiert"));
  ...
  void returnsConflictWhenSeedReferencesPreventDelete() throws Exception {
    ...
    mvc.perform(get(BASE + "/1"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.isbn").value("9783446279899"));
  }
}
