...
`ExhibitController` dokumentiert die Liste, die Einzelabfrage und POST unter
`/api/exhibits`. Die GET-Antworten verwenden `ExhibitResponse`. POST nimmt
`CreateExhibitRequest` entgegen und liefert bei Erfolg 201 mit einem `Location`-Header.
Wenn `inventoryCode` bereits vergeben ist, beschreibt der Vertrag eine
409-Antwort mit `ApiError`.

Die `@Schema`-Annotationen an beiden DTOs erklären jedes Feld und enthalten
konkrete Beispiele, darunter `inventoryCode`. SpringDoc übernimmt Java-Typen und
