...
class ExhibitApiIntegrationTest {
  private static final String BASE = "/api/exhibits";
  private static final String VALID =
      "{\"inventoryCode\": \"EX-99\", \"title\": \"Testexponat\", \"insuredValue\": 1000}";
  ...
  void readsSeedDataAndReportsMissingIds() throws Exception {
    ...
    mvc.perform(get(BASE + "/999999").header("X-Correlation-ID", "read-404"))
        ...
        .andExpect(jsonPath("$.code").value("EXHIBIT_NOT_FOUND"))
  ...
  void createsReplacesAndDeletesThroughRealPersistence() throws Exception {
    var created =
        mvc.perform(post(BASE).contentType(MediaType.APPLICATION_JSON).content(VALID))
            ...
            .andExpect(jsonPath("$.inventoryCode").value("EX-99"))
            .andReturn();
    ...
    mvc.perform(
            put(BASE + "/" + id)
                ...
                .content(
                    "{\"inventoryCode\": \"EX-99\", \"title\": \"Testexponat aktualisiert\","
                        + " \"insuredValue\": 1000}"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.id").value(id))
        .andExpect(jsonPath("$.title").value("Testexponat aktualisiert"));
    ...
    mvc.perform(
            put(BASE + "/" + id)
                .contentType(MediaType.APPLICATION_JSON)
                .content(
                    "{\"inventoryCode\": \"EX-01\", \"title\": \"Testexponat aktualisiert\","
                        + " \"insuredValue\": 1000}"))
  ...
  void returnsConflictWhenSeedReferencesPreventDelete() throws Exception {
    ...
    mvc.perform(get(BASE + "/1"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.inventoryCode").value("EX-01"));
  }
}
