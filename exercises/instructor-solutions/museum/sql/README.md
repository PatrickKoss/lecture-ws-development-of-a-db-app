...
```sql
UPDATE exhibits
SET inventory_code = (SELECT inventory_code FROM exhibits WHERE id = 1)
WHERE id = 2;
-- UNIQUE constraint failed: exhibits.inventory_code
```
...
UPDATE exhibits SET gallery_id = 999999 WHERE id = (SELECT id FROM exhibits LIMIT 1);
