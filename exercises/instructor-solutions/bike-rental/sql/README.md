...
```sql
UPDATE bikes
SET bike_number = (SELECT bike_number FROM bikes WHERE id = 1)
WHERE id = 2;
-- UNIQUE constraint failed: bikes.bike_number
```
...
UPDATE rentals SET customer_id = 999999 WHERE id = (SELECT id FROM rentals LIMIT 1);
