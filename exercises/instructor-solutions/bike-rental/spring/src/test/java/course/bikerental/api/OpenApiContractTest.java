  ...
  void documentsAndExportsTheImplementedContract() throws Exception {
    var result =
        mvc.perform(get("/v3/api-docs"))
            ...
            .andExpect(jsonPath("$.paths['/api/stations'].post").exists())
            .andExpect(jsonPath("$.paths['/api/stations/{id}'].get").exists())
            .andExpect(jsonPath("$.paths['/api/stations/{id}'].put").exists())
            .andExpect(jsonPath("$.paths['/api/stations/{id}'].delete").exists())
            .andExpect(
                jsonPath("$.components.schemas.CreateStationRequest.properties.id").doesNotExist())
            .andExpect(
                jsonPath("$.components.schemas.StationResponse.properties.id.readOnly").value(true))
            .andReturn();
