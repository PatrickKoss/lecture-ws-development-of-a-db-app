...
`StationController` dokumentiert die Liste, die Einzelabfrage und POST unter
`/api/stations`. Die GET-Antworten verwenden `StationResponse`. POST nimmt
`CreateStationRequest` entgegen und liefert bei Erfolg 201 mit einem `Location`-Header.
Wenn `stationCode` bereits vergeben ist, beschreibt der Vertrag eine
409-Antwort mit `ApiError`.

Die `@Schema`-Annotationen an beiden DTOs erklären jedes Feld und enthalten
konkrete Beispiele, darunter `stationCode`. SpringDoc übernimmt Java-Typen und
