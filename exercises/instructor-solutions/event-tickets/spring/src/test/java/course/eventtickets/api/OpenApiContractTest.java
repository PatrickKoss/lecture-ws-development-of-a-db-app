  ...
  void documentsAndExportsTheImplementedContract() throws Exception {
    var result =
        mvc.perform(get("/v3/api-docs"))
            ...
            .andExpect(jsonPath("$.paths['/api/venues'].post").exists())
            .andExpect(jsonPath("$.paths['/api/venues/{id}'].get").exists())
            .andExpect(jsonPath("$.paths['/api/venues/{id}'].put").exists())
            .andExpect(jsonPath("$.paths['/api/venues/{id}'].delete").exists())
            .andExpect(
                jsonPath("$.components.schemas.CreateVenueRequest.properties.id").doesNotExist())
            .andExpect(
                jsonPath("$.components.schemas.VenueResponse.properties.id.readOnly").value(true))
            .andReturn();
