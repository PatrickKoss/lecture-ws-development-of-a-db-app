...
class VenueApiIntegrationTest {
  private static final String BASE = "/api/venues";
  ...
  void createsReplacesAndDeletesThroughRealPersistence() throws Exception {
    var created =
        mvc.perform(post(BASE).contentType(MediaType.APPLICATION_JSON).content(VALID))
            ...
            .andExpect(jsonPath("$.venueCode").value("V-99"))
            .andReturn();
    ...
    mvc.perform(
            put(BASE + "/" + id)
                ...
                .content(
                    "{\"venueCode\": \"V-99\", \"name\": \"Testhalle aktualisiert\", \"street\":"
                        + " \"Testweg 1\", \"postalCode\": \"45127\", \"city\": \"Essen\","
                        + " \"capacity\": 300}"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.id").value(id))
        .andExpect(jsonPath("$.name").value("Testhalle aktualisiert"));
    ...
    mvc.perform(
            put(BASE + "/" + id)
                .contentType(MediaType.APPLICATION_JSON)
                .content(
                    "{\"venueCode\": \"V-01\", \"name\": \"Testhalle aktualisiert\", \"street\":"
                        + " \"Testweg 1\", \"postalCode\": \"45127\", \"city\": \"Essen\","
                        + " \"capacity\": 300}"))
  ...
  void returnsConflictWhenSeedReferencesPreventDelete() throws Exception {
    ...
    mvc.perform(get(BASE + "/1"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.venueCode").value("V-01"));
  }
}
