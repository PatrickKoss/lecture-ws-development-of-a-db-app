...
`VenueController` dokumentiert die Liste, die Einzelabfrage und POST unter
`/api/venues`. Die GET-Antworten verwenden `VenueResponse`. POST nimmt
`CreateVenueRequest` entgegen und liefert bei Erfolg 201 mit einem `Location`-Header.
Wenn `venueCode` bereits vergeben ist, beschreibt der Vertrag eine
409-Antwort mit `ApiError`.

Die `@Schema`-Annotationen an beiden DTOs erklären jedes Feld und enthalten
konkrete Beispiele, darunter `venueCode`. SpringDoc übernimmt Java-Typen und
