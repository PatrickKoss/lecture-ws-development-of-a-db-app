...
class ParcelApiIntegrationTest {
  private static final String BASE = "/api/parcels";
  private static final String VALID =
      "{\"trackingCode\": \"PK-99\", \"recipient\": \"Test Empfänger\", \"weight\": 3.5}";
  ...
  void createsReplacesAndDeletesThroughRealPersistence() throws Exception {
    var created =
        mvc.perform(post(BASE).contentType(MediaType.APPLICATION_JSON).content(VALID))
            ...
            .andExpect(jsonPath("$.trackingCode").value("PK-99"))
            .andReturn();
    ...
    mvc.perform(
            put(BASE + "/" + id)
                ...
                .content(
                    "{\"trackingCode\": \"PK-99\", \"recipient\": \"Test Empfänger aktualisiert\","
                        + " \"weight\": 3.5}"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.id").value(id))
        .andExpect(jsonPath("$.recipient").value("Test Empfänger aktualisiert"));
    ...
    mvc.perform(
            put(BASE + "/" + id)
                .contentType(MediaType.APPLICATION_JSON)
                .content(
                    "{\"trackingCode\": \"PK-01\", \"recipient\": \"Test Empfänger aktualisiert\","
                        + " \"weight\": 3.5}"))
