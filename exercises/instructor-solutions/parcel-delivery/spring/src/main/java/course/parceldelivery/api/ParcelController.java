...
public class ParcelController {
  ...
  @ApiResponse(
      responseCode = "409",
      description = "tracking_code ist vergeben oder eine Datenbankregel wird verletzt",
      content = @Content(schema = @Schema(implementation = ApiError.class)))
  @ApiResponse(
      responseCode = "415",
  ...
  public ResponseEntity<ParcelResponse> create(@Valid @RequestBody CreateParcelRequest request) {
    var response = ParcelResponse.from(service.create(request.toCommand()));
    var location =
  ...
  @Operation(summary = "Parcel vollständig ersetzen")
  @ApiResponse(
      responseCode = "200",
  ...
  public ParcelResponse replace(
      @PathVariable @Positive long id, @Valid @RequestBody UpdateParcelRequest request) {
    return ParcelResponse.from(service.replace(id, request.toCommand()));
  }

  @Operation(
      summary = "Parcel löschen",
