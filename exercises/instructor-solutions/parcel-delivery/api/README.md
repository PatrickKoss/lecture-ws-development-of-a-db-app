...
`ParcelController` dokumentiert die Liste, die Einzelabfrage und POST unter
`/api/parcels`. Die GET-Antworten verwenden `ParcelResponse`. POST nimmt
`CreateParcelRequest` entgegen und liefert bei Erfolg 201 mit einem `Location`-Header.
Wenn `trackingCode` bereits vergeben ist, beschreibt der Vertrag eine
409-Antwort mit `ApiError`.

Die `@Schema`-Annotationen an beiden DTOs erklären jedes Feld und enthalten
konkrete Beispiele, darunter `trackingCode`. SpringDoc übernimmt Java-Typen und
