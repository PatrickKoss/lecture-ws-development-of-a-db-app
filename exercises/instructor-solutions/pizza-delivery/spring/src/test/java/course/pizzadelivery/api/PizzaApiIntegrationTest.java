...
class PizzaApiIntegrationTest {
  private static final String BASE = "/api/pizzas";
  private static final String VALID =
      "{\"pizzaNumber\": \"P-99\", \"name\": \"Testpizza\", \"category\": \"Saisonal\","
          + " \"ovenStation\": \"OFEN-Z\", \"basePrice\": 11.5, \"active\": true}";
  ...
  void readsSeedDataAndReportsMissingIds() throws Exception {
    ...
    mvc.perform(get(BASE + "/999999").header("X-Correlation-ID", "read-404"))
        ...
        .andExpect(jsonPath("$.code").value("PIZZA_NOT_FOUND"))
  ...
  void createsReplacesAndDeletesThroughRealPersistence() throws Exception {
    var created =
        mvc.perform(post(BASE).contentType(MediaType.APPLICATION_JSON).content(VALID))
            ...
            .andExpect(jsonPath("$.pizzaNumber").value("P-99"))
            .andReturn();
    ...
    mvc.perform(
            put(BASE + "/" + id)
                ...
                .content(
                    "{\"pizzaNumber\": \"P-99\", \"name\": \"Testpizza aktualisiert\","
                        + " \"category\": \"Saisonal\", \"ovenStation\": \"OFEN-Z\", \"basePrice\":"
                        + " 11.5, \"active\": true}"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.id").value(id))
        .andExpect(jsonPath("$.name").value("Testpizza aktualisiert"));
    ...
    mvc.perform(
            put(BASE + "/" + id)
                .contentType(MediaType.APPLICATION_JSON)
                .content(
                    "{\"pizzaNumber\": \"P-01\", \"name\": \"Testpizza aktualisiert\","
                        + " \"category\": \"Saisonal\", \"ovenStation\": \"OFEN-Z\", \"basePrice\":"
                        + " 11.5, \"active\": true}"))
  ...
  void returnsConflictWhenSeedReferencesPreventDelete() throws Exception {
    ...
    mvc.perform(get(BASE + "/1"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.pizzaNumber").value("P-01"));
  }
}
