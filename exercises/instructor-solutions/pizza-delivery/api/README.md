...
`PizzaController` dokumentiert die Liste, die Einzelabfrage und POST unter
`/api/pizzas`. Die GET-Antworten verwenden `PizzaResponse`. POST nimmt
`CreatePizzaRequest` entgegen und liefert bei Erfolg 201 mit einem `Location`-Header.
Wenn `pizzaNumber` bereits vergeben ist, beschreibt der Vertrag eine
409-Antwort mit `ApiError`.

Die `@Schema`-Annotationen an beiden DTOs erklären jedes Feld und enthalten
konkrete Beispiele, darunter `pizzaNumber`. SpringDoc übernimmt Java-Typen und
