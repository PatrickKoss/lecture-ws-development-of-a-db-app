...
```sql
UPDATE pizzas
SET pizza_number = (SELECT pizza_number FROM pizzas WHERE id = 1)
WHERE id = 2;
-- UNIQUE constraint failed: pizzas.pizza_number
```
...
UPDATE order_items SET pizza_id = 999999 WHERE rowid = (SELECT rowid FROM order_items LIMIT 1);
