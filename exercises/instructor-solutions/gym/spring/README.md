...
Der Controller wandelt Request-DTOs in `CourseCommand` um und gibt nur Response-DTOs zurück. `CourseService` setzt die Transaktionsgrenzen und prüft fachliche Konflikte. `CourseRepository` ist die vom Service verwendete Abstraktion. `JpaCourseRepository` und die Spring-Data-Schnittstelle bilden die Persistenzschicht. Hibernate mappt die Domainwerte auf `courses`; im Anwendungscode steht kein SQL. Auch das vorbereitete Lookup liest über Spring Data JPA.
