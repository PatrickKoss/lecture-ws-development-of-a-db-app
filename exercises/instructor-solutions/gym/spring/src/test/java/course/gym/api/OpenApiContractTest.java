  ...
  void documentsAndExportsTheImplementedContract() throws Exception {
    var result =
        mvc.perform(get("/v3/api-docs"))
            ...
            .andExpect(jsonPath("$.paths['/api/courses/{id}'].put").exists())
            .andExpect(jsonPath("$.paths['/api/courses/{id}'].delete").exists())
            .andExpect(
                jsonPath("$.components.schemas.CreateCourseRequest.properties.id").doesNotExist())
            .andExpect(
                jsonPath("$.components.schemas.CourseResponse.properties.id.readOnly").value(true))
            .andReturn();
