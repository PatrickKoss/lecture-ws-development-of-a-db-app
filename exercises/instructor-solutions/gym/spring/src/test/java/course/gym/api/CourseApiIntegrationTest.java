...
class CourseApiIntegrationTest {
  ...
  private static final String VALID =
      "{\"courseCode\": \"C-999\", \"title\": \"Mobility Lab\", \"level\": \"BEGINNER\","
  ...
  void createsReplacesAndDeletesThroughRealPersistence() throws Exception {
    var created =
        mvc.perform(post(BASE).contentType(MediaType.APPLICATION_JSON).content(VALID))
            ...
            .andExpect(jsonPath("$.courseCode").value("C-999"))
            .andReturn();
    ...
    mvc.perform(
            put(BASE + "/" + id)
                ...
                .content(
                    "{\"courseCode\": \"C-999\", \"title\": \"Mobility Lab aktualisiert\","
                        + " \"level\": \"BEGINNER\", \"durationMinutes\": 45, \"roomId\": 1}"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.id").value(id))
        .andExpect(jsonPath("$.title").value("Mobility Lab aktualisiert"));
    ...
    mvc.perform(
            put(BASE + "/" + id)
                .contentType(MediaType.APPLICATION_JSON)
                .content(
                    "{\"courseCode\": \"C-101\", \"title\": \"Mobility Lab aktualisiert\","
                        + " \"level\": \"BEGINNER\", \"durationMinutes\": 45, \"roomId\": 1}"))
  ...
  @Test
  void mapsForeignKeyFailureToConflictWithoutDatabaseDetails() throws Exception {
    mvc.perform(
            post(BASE)
                .contentType(MediaType.APPLICATION_JSON)
                .content(
                    "{\"courseCode\": \"C-998\", \"title\": \"Raumloser Kurs\", \"level\":"
                        + " \"BEGINNER\", \"durationMinutes\": 45, \"roomId\": 999999}"))
        .andExpect(status().isConflict())
        .andExpect(jsonPath("$.code").value("DATA_INTEGRITY_VIOLATION"))
        .andExpect(jsonPath("$.message").value("Die Änderung verletzt eine Datenbankregel"));
  }
  ...
  void returnsConflictWhenSeedReferencesPreventDelete() throws Exception {
    ...
    mvc.perform(get(BASE + "/1"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.courseCode").value("C-101"));
  }
}
