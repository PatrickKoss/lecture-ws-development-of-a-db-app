...
```sql
UPDATE courses
SET course_code = (SELECT course_code FROM courses WHERE id = 1)
WHERE id = 2;
-- UNIQUE constraint failed: courses.course_code
```
...
UPDATE course_sessions SET course_id = 999999 WHERE id = (SELECT id FROM course_sessions LIMIT 1);
