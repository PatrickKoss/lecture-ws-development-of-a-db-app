...
class ResourceApiIntegrationTest {
  private static final String BASE = "/api/resources";
  private static final String VALID =
      "{\"resourceCode\": \"R-99\", \"name\": \"Neue Ressource\", \"measure\": 12.5}";
  ...
  void createsReplacesAndDeletesThroughRealPersistence() throws Exception {
    var created =
        mvc.perform(post(BASE).contentType(MediaType.APPLICATION_JSON).content(VALID))
            ...
            .andExpect(jsonPath("$.resourceCode").value("R-99"))
            .andReturn();
    ...
    mvc.perform(
            put(BASE + "/" + id)
                ...
                .content(
                    "{\"resourceCode\": \"R-99\", \"name\": \"Neue Ressource aktualisiert\","
                        + " \"measure\": 12.5}"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.id").value(id))
        .andExpect(jsonPath("$.name").value("Neue Ressource aktualisiert"));
    ...
    mvc.perform(
            put(BASE + "/" + id)
                .contentType(MediaType.APPLICATION_JSON)
                .content(
                    "{\"resourceCode\": \"R-01\", \"name\": \"Neue Ressource aktualisiert\","
                        + " \"measure\": 12.5}"))
