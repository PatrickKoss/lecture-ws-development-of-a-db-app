  ...
  void documentsAndExportsTheImplementedContract() throws Exception {
    var result =
        mvc.perform(get("/v3/api-docs"))
            ...
            .andExpect(jsonPath("$.paths['/api/resources'].post").exists())
            .andExpect(jsonPath("$.paths['/api/resources/{id}'].get").exists())
            .andExpect(jsonPath("$.paths['/api/resources/{id}'].put").exists())
            .andExpect(jsonPath("$.paths['/api/resources/{id}'].delete").exists())
            .andExpect(
                jsonPath("$.components.schemas.CreateResourceRequest.properties.id").doesNotExist())
            .andExpect(
                jsonPath("$.components.schemas.ResourceResponse.properties.id.readOnly")
                    .value(true))
            .andReturn();
