...
`MovieController` dokumentiert die Liste, die Einzelabfrage und POST unter
`/api/movies`. Die GET-Antworten verwenden `MovieResponse`. POST nimmt
`CreateMovieRequest` entgegen und liefert bei Erfolg 201 mit einem `Location`-Header.
Wenn `movieCode` bereits vergeben ist, beschreibt der Vertrag eine
409-Antwort mit `ApiError`.

Die `@Schema`-Annotationen an beiden DTOs erklären jedes Feld und enthalten
konkrete Beispiele, darunter `movieCode`. SpringDoc übernimmt Java-Typen und
