...
class MovieApiIntegrationTest {
  ...
  void createsReplacesAndDeletesThroughRealPersistence() throws Exception {
    ...
    mvc.perform(
            put(BASE + "/" + id)
                ...
                .content(
                    "{\"movieCode\": \"F-999\", \"title\": \"Testfilm aktualisiert\","
                        + " \"releaseYear\": 2026, \"durationMinutes\": 90, \"fskCode\": \"FSK_6\","
                        + " \"minimumAge\": 6}"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.id").value(id))
        .andExpect(jsonPath("$.title").value("Testfilm aktualisiert"));
    ...
    mvc.perform(
            put(BASE + "/" + id)
                .contentType(MediaType.APPLICATION_JSON)
                .content(
                    "{\"movieCode\": \"F-101\", \"title\": \"Testfilm aktualisiert\","
                        + " \"releaseYear\": 2026, \"durationMinutes\": 90, \"fskCode\": \"FSK_6\","
                        + " \"minimumAge\": 6}"))
        .andExpect(status().isConflict())
        .andExpect(jsonPath("$.code").value("MOVIE_CODE_EXISTS"));
    mvc.perform(get(BASE + "/" + id))
        .andExpect(jsonPath("$.movieCode").value("F-999"))
        .andExpect(jsonPath("$.title").value("Testfilm aktualisiert"));
  ...
  void returnsConflictWhenSeedReferencesPreventDelete() throws Exception {
    ...
    mvc.perform(get(BASE + "/1"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.movieCode").value("F-101"));
  }
}
