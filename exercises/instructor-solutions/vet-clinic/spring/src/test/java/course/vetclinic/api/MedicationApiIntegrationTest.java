...
class MedicationApiIntegrationTest {
  private static final String BASE = "/api/medications";
  private static final String VALID =
      "{\"pzn\": \"00999999\", \"productName\": \"Testpräparat\", \"activeIngredient\":"
          + " \"Teststoff\", \"dosageForm\": \"Tablette\", \"prescriptionRequired\": true,"
          + " \"active\": true}";
  ...
  void createsReplacesAndDeletesThroughRealPersistence() throws Exception {
    ...
    mvc.perform(
            put(BASE + "/" + id)
                ...
                .content(
                    "{\"pzn\": \"00999999\", \"productName\": \"Testpräparat aktualisiert\","
                        + " \"activeIngredient\": \"Teststoff\", \"dosageForm\": \"Tablette\","
                        + " \"prescriptionRequired\": true, \"active\": true}"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.id").value(id))
        .andExpect(jsonPath("$.productName").value("Testpräparat aktualisiert"));
    ...
    mvc.perform(
            put(BASE + "/" + id)
                .contentType(MediaType.APPLICATION_JSON)
                .content(
                    "{\"pzn\": \"09231122\", \"productName\": \"Testpräparat aktualisiert\","
                        + " \"activeIngredient\": \"Teststoff\", \"dosageForm\": \"Tablette\","
                        + " \"prescriptionRequired\": true, \"active\": true}"))
  ...
  void returnsConflictWhenSeedReferencesPreventDelete() throws Exception {
    ...
    mvc.perform(get(BASE + "/1"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.pzn").value("09231122"));
  }
}
