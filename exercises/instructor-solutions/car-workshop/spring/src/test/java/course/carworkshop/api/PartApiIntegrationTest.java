...
class PartApiIntegrationTest {
  ...
  private static final String VALID =
      "{\"partNumber\": \"P-9999\", \"name\": \"Testfilter\", \"category\": \"Filter\","
          + " \"shelfCode\": \"T-01\", \"stockQuantity\": 5, \"reorderLevel\": 2, \"listPrice\":"
          + " 14.9}";
  ...
  void createsReplacesAndDeletesThroughRealPersistence() throws Exception {
    var created =
        mvc.perform(post(BASE).contentType(MediaType.APPLICATION_JSON).content(VALID))
            ...
            .andExpect(jsonPath("$.partNumber").value("P-9999"))
            .andReturn();
    ...
    mvc.perform(
            put(BASE + "/" + id)
                ...
                .content(
                    "{\"partNumber\": \"P-9999\", \"name\": \"Testfilter aktualisiert\","
                        + " \"category\": \"Filter\", \"shelfCode\": \"T-01\", \"stockQuantity\":"
                        + " 5, \"reorderLevel\": 2, \"listPrice\": 14.9}"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.id").value(id))
        .andExpect(jsonPath("$.name").value("Testfilter aktualisiert"));
    ...
    mvc.perform(
            put(BASE + "/" + id)
                .contentType(MediaType.APPLICATION_JSON)
                .content(
                    "{\"partNumber\": \"P-1001\", \"name\": \"Testfilter aktualisiert\","
                        + " \"category\": \"Filter\", \"shelfCode\": \"T-01\", \"stockQuantity\":"
                        + " 5, \"reorderLevel\": 2, \"listPrice\": 14.9}"))
  ...
  void returnsConflictWhenSeedReferencesPreventDelete() throws Exception {
    ...
    mvc.perform(get(BASE + "/1"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.partNumber").value("P-1001"));
  }
}
