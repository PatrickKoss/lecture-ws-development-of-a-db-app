  ...
  void documentsAndExportsTheImplementedContract() throws Exception {
    var result =
        mvc.perform(get("/v3/api-docs"))
            ...
            .andExpect(jsonPath("$.paths['/api/parts/{id}'].put").exists())
            .andExpect(jsonPath("$.paths['/api/parts/{id}'].delete").exists())
            .andExpect(
                jsonPath("$.components.schemas.CreatePartRequest.properties.id").doesNotExist())
            .andExpect(
                jsonPath("$.components.schemas.PartResponse.properties.id.readOnly").value(true))
            .andReturn();
