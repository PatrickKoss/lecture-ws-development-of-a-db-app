...
```sql
UPDATE restaurants
SET partner_number = (SELECT partner_number FROM restaurants WHERE id = 1)
WHERE id = 2;
-- UNIQUE constraint failed: restaurants.partner_number
```
...
UPDATE dishes SET restaurant_id = 999999 WHERE id = (SELECT id FROM dishes LIMIT 1);
