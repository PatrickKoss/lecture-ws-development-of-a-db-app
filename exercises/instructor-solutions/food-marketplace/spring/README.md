# Spring-Lösung: food-marketplace
...
Die API liegt unter `/api/restaurants`. Sie unterstützt Liste, Lesen nach ID, POST, vollständiges PUT und idempotentes DELETE. `requests.http` enthält passende Aufrufe. SpringDoc stellt Swagger UI unter `/swagger-ui.html` und das JSON-Dokument unter `/v3/api-docs` bereit. Der OpenAPI-Test schreibt denselben generierten Vertrag nach `build/openapi/openapi.json`.

Der Controller wandelt Request-DTOs in `RestaurantCommand` um und gibt nur Response-DTOs zurück. `RestaurantService` setzt die Transaktionsgrenzen und prüft fachliche Konflikte. `RestaurantRepository` ist die vom Service verwendete Abstraktion. `JpaRestaurantRepository` und die Spring-Data-Schnittstelle bilden die Persistenzschicht. Hibernate mappt die Domainwerte auf `restaurants`; im Anwendungscode steht kein SQL. Auch das vorbereitete Lookup liest über Spring Data JPA.
