  ...
  void documentsAndExportsTheImplementedContract() throws Exception {
    var result =
        mvc.perform(get("/v3/api-docs"))
            ...
            .andExpect(jsonPath("$.paths['/api/restaurants/{id}'].put").exists())
            .andExpect(jsonPath("$.paths['/api/restaurants/{id}'].delete").exists())
            .andExpect(
                jsonPath("$.components.schemas.CreateRestaurantRequest.properties.id")
                    .doesNotExist())
            .andExpect(
                jsonPath("$.components.schemas.RestaurantResponse.properties.id.readOnly")
                    .value(true))
            .andReturn();
