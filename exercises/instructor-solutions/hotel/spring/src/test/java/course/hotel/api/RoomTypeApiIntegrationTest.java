...
class RoomTypeApiIntegrationTest {
  private static final String BASE = "/api/room-types";
  private static final String VALID =
      "{\"typeCode\": \"NX\", \"name\": \"Testzimmer\", \"capacity\": 2, \"standardPriceCents\":"
          + " 14900}";
  ...
  void createsReplacesAndDeletesThroughRealPersistence() throws Exception {
    var created =
        mvc.perform(post(BASE).contentType(MediaType.APPLICATION_JSON).content(VALID))
            ...
            .andExpect(jsonPath("$.typeCode").value("NX"))
            .andReturn();
    ...
    mvc.perform(
            put(BASE + "/" + id)
                ...
                .content(
                    "{\"typeCode\": \"NX\", \"name\": \"Testzimmer aktualisiert\", \"capacity\": 2,"
                        + " \"standardPriceCents\": 14900}"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.id").value(id))
        .andExpect(jsonPath("$.name").value("Testzimmer aktualisiert"));
    ...
    mvc.perform(
            put(BASE + "/" + id)
                .contentType(MediaType.APPLICATION_JSON)
                .content(
                    "{\"typeCode\": \"EZ\", \"name\": \"Testzimmer aktualisiert\", \"capacity\": 2,"
                        + " \"standardPriceCents\": 14900}"))
  ...
  void returnsConflictWhenSeedReferencesPreventDelete() throws Exception {
    ...
    mvc.perform(get(BASE + "/1"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.typeCode").value("EZ"));
  }
}
