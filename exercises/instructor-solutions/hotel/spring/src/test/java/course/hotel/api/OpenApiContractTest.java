  ...
  void documentsAndExportsTheImplementedContract() throws Exception {
    var result =
        mvc.perform(get("/v3/api-docs"))
            ...
            .andExpect(jsonPath("$.paths['/api/room-types'].post").exists())
            .andExpect(jsonPath("$.paths['/api/room-types/{id}'].get").exists())
            .andExpect(jsonPath("$.paths['/api/room-types/{id}'].put").exists())
            .andExpect(jsonPath("$.paths['/api/room-types/{id}'].delete").exists())
            .andExpect(
                jsonPath("$.components.schemas.CreateRoomTypeRequest.properties.id").doesNotExist())
            .andExpect(
                jsonPath("$.components.schemas.RoomTypeResponse.properties.id.readOnly")
                    .value(true))
            .andReturn();
