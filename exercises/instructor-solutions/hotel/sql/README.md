...
```sql
UPDATE room_types
SET type_code = (SELECT type_code FROM room_types WHERE id = 1)
WHERE id = 2;
-- UNIQUE constraint failed: room_types.type_code
```
...
UPDATE rooms SET room_type_id = 999999 WHERE rowid = (SELECT rowid FROM rooms LIMIT 1);
