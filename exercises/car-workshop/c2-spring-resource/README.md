...
Die fachliche Klasse `Part` bleibt ein unveränderlicher Java-Record. `PartJpaEntity` bildet die Tabelle für JPA ab. `SpringDataPartRepository` erweitert `JpaRepository` und ist fertig. Offen sind `findAll` und `findById` im Adapter `JpaPartRepository`. Das vorbereitete `JpaLookupRepository` liest `mechanics`.
...
1. Implementiert `findAll` in `JpaPartRepository`. Nutzt das Spring-Data-Repository und mappt jede `PartJpaEntity` auf `Part`.
...
4. Startet die Anwendung und prüft GET-Liste sowie GET nach ID über Swagger UI. Der Controller liefert `PartResponse`, keine JPA-Entity. Eine unbekannte ID ergibt 404.
...
Die Anwendung startet. Beide GET-Endpunkte liefern die Seed-Daten über das Response-DTO. ID 1 enthält `P-1001`. `JpaRepositoryExerciseTest` und `OpenApiStarterTest` laufen.
...
Welche Aufgabe hat `PartJpaEntity`? Warum gibt der Controller trotzdem `PartResponse` zurück? Wo endet Spring Data und wo beginnt euer Adapter?
