...
`Part` ist der Domänen-Record. `PartJpaEntity` bildet die Tabelle für JPA ab. `SpringDataPartRepository` und die Infrastruktur sind vorbereitet. Der Controller, der Service und `JpaPartRepository` enthalten die TODOs für beide GET-Wege. `JpaLookupRepository` liest `mechanics`.
...
1. Implementiert `PartResponse.from(Part)`.
2. Implementiert `findAll` in `JpaPartRepository`. Nutzt das Spring-Data-Repository, sortiert nach ID und mappt jede `PartJpaEntity` auf `Part`.
3. Implementiert `findAll` im Service und danach im Controller. Der Controller mappt Domänenobjekte auf `PartResponse`.
...
Beide GET-Endpunkte lesen über Service, Repository-Interface und JPA-Adapter. Der Controller gibt `PartResponse` zurück. Eine unbekannte ID ergibt 404. Die aktivierten Repository- und API-Checkpoints laufen.
