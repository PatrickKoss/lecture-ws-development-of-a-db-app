...
Die aufrufende Schicht kennt nur `PartRepository` und die Domänenklasse `Part`. Eine Quelltextsuche findet JDBC-Typen nur in `Database` und JDBC-Repositorys. Wenn ihr die Vertiefung bearbeitet habt, läuft der Catalog-Test zusätzlich mit der In-Memory-Implementierung.
...
C1 bis C3 verwenden ein vorbereitetes, eigenständiges Spring-Boot-Projekt. Übernehmt Schema und Seed-Daten als Flyway-Migrationen. Übertragt die Repository-Idee, aber kopiert `JdbcPartRepository` nicht in den Spring-Starter. In C1 erstellt ihr DTOs und HTTP-Vertrag. C2 setzt die Lesewege durch Controller, Service und JPA-Adapter um. Spring Data führt die Datenbankabfragen über JPA und Hibernate aus.
