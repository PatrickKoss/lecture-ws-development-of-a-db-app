...
Die fachliche Klasse `Course` bleibt ein unveränderlicher Java-Record. `CourseJpaEntity` bildet die Tabelle für JPA ab. `SpringDataCourseRepository` erweitert `JpaRepository` und ist fertig. Offen sind `findAll` und `findById` im Adapter `JpaCourseRepository`. Das vorbereitete `JpaLookupRepository` liest `trainers`.
...
1. Implementiert `findAll` in `JpaCourseRepository`. Nutzt das Spring-Data-Repository und mappt jede `CourseJpaEntity` auf `Course`.
...
4. Startet die Anwendung und prüft GET-Liste sowie GET nach ID über Swagger UI. Der Controller liefert `CourseResponse`, keine JPA-Entity. Eine unbekannte ID ergibt 404.
...
Die Anwendung startet. Beide GET-Endpunkte liefern die Seed-Daten über das Response-DTO. ID 1 enthält `C-101`. `JpaRepositoryExerciseTest` und `OpenApiStarterTest` laufen.
...
Welche Aufgabe hat `CourseJpaEntity`? Warum gibt der Controller trotzdem `CourseResponse` zurück? Wo endet Spring Data und wo beginnt euer Adapter?
