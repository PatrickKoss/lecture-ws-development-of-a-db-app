...
class OpenApiTest {
  ...
  void createsResponseWithLocation() throws Exception {
    var value = mock(Course.class);
    when(value.id()).thenReturn(42L);
    when(service.create(any(CreateCourseRequest.class))).thenReturn(value);
    mvc.perform(post("/api/courses").contentType("application/json")
            .content("{\"courseCode\": \"C-199\", \"title\": \"Kursformat\", \"level\": \"BEGINNER\", \"durationMinutes\": 45, \"roomId\": 1}"))
