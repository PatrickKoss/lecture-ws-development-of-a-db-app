...
Nutzt das Modell `Course` aus B3 und das Projekt `../c2-spring-resource/starter`.
SpringDoc ist eingebunden. `CourseController` enthält ein dokumentiertes POST-Beispiel mit `CreateCourseRequest`, `CourseResponse`, 201, `Location`, 400 und 409.
...
2. Ergänzt Beschreibungen und passende Beispiele an den Feldern von `CreateCourseRequest` und `CourseResponse` mit `@Schema`. Beachtet die vorhandenen Validierungsregeln. Der Request enthält keine Server-ID.
3. Dokumentiert beide GET-Methoden mit `@Operation` und `@ApiResponse`. Die Liste liefert 200 mit einem Array aus `CourseResponse`, GET nach ID liefert 200 oder 404 mit `ApiError`. Für das Array nutzt ihr `@Content(array = @ArraySchema(schema = @Schema(implementation = CourseResponse.class)))` und importiert `io.swagger.v3.oas.annotations.media.ArraySchema`.
