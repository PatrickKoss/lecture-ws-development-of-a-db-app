...
Die fachliche Klasse `MusicCourse` bleibt ein unveränderlicher Java-Record. `MusicCourseJpaEntity` bildet die Tabelle für JPA ab. `SpringDataMusicCourseRepository` erweitert `JpaRepository` und ist fertig. Offen sind `findAll` und `findById` im Adapter `JpaMusicCourseRepository`. Das vorbereitete `JpaLookupRepository` liest `rooms`.
...
1. Implementiert `findAll` in `JpaMusicCourseRepository`. Nutzt das Spring-Data-Repository und mappt jede `MusicCourseJpaEntity` auf `MusicCourse`.
...
4. Startet die Anwendung und prüft GET-Liste sowie GET nach ID über Swagger UI. Der Controller liefert `MusicCourseResponse`, keine JPA-Entity. Eine unbekannte ID ergibt 404.
...
Die Anwendung startet. Beide GET-Endpunkte liefern die Seed-Daten über das Response-DTO. ID 1 enthält `MU-01`. `JpaRepositoryExerciseTest` und `OpenApiStarterTest` laufen.
...
Welche Aufgabe hat `MusicCourseJpaEntity`? Warum gibt der Controller trotzdem `MusicCourseResponse` zurück? Wo endet Spring Data und wo beginnt euer Adapter?
