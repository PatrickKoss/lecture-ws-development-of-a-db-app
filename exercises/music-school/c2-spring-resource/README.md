...
`MusicCourse` ist der Domänen-Record. `MusicCourseJpaEntity` bildet die Tabelle für JPA ab. `SpringDataMusicCourseRepository` und die Infrastruktur sind vorbereitet. Der Controller, der Service und `JpaMusicCourseRepository` enthalten die TODOs für beide GET-Wege. `JpaLookupRepository` liest `rooms`.
...
1. Implementiert `MusicCourseResponse.from(MusicCourse)`.
2. Implementiert `findAll` in `JpaMusicCourseRepository`. Nutzt das Spring-Data-Repository, sortiert nach ID und mappt jede `MusicCourseJpaEntity` auf `MusicCourse`.
3. Implementiert `findAll` im Service und danach im Controller. Der Controller mappt Domänenobjekte auf `MusicCourseResponse`.
...
Beide GET-Endpunkte lesen über Service, Repository-Interface und JPA-Adapter. Der Controller gibt `MusicCourseResponse` zurück. Eine unbekannte ID ergibt 404. Die aktivierten Repository- und API-Checkpoints laufen.
