...
Implementiert `insert`, `existsByCourseCode` und prüft die vorhandene POST-Methode. Aktiviert den Controller-Test. Prüft 201 samt `Location`-Header, einen Validierungsfehler mit 400 und den doppelten `course_code` mit 409. Der Fehlerkörper enthält `code`, `message`, `correlationId` und bei Validierung `fields`.
