...
- courseCode: Text, 1 bis 40 Zeichen, nicht leer
- title: Text, 1 bis 160 Zeichen, nicht leer
- fee: Dezimalzahl von 0 bis 500
...
1. Ergänzt die Validierungsannotationen in `CreateMusicCourseRequest` und implementiert `toCommand()`. Die ID gehört nicht in den Request.
2. Implementiert `existsByCourseCode(String)` und `save(MusicCourse)` in `JpaMusicCourseRepository`. Nutzt die vorbereitete Spring-Data-Methode. Mappt beim Speichern auf `MusicCourseJpaEntity`, verwendet `saveAndFlush` und reicht Datenbankfehler an `SQLiteConstraintTranslator` weiter.
...
4. Implementiert POST im Controller. Validiert den Request, ruft den Service auf und liefert 201 mit dem gespeicherten `MusicCourseResponse` sowie einem `Location`-Header auf die neue Ressource.
