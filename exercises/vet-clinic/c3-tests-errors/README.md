...
- pzn: genau acht Ziffern
- productName: Text, 1 bis 120 Zeichen, nicht leer
- activeIngredient: Text, 1 bis 160 Zeichen, nicht leer
- dosageForm: Text, 1 bis 50 Zeichen, nicht leer
- PrescriptionRequired und active: Wahrheitswerte, erforderlich
...
1. Ergänzt die Validierungsannotationen in `CreateMedicationRequest` und implementiert `toCommand()`. Die ID gehört nicht in den Request.
2. Implementiert `existsByPzn(String)` und `save(Medication)` in `JpaMedicationRepository`. Nutzt die vorbereitete Spring-Data-Methode. Mappt beim Speichern auf `MedicationJpaEntity`, verwendet `saveAndFlush` und reicht Datenbankfehler an `SQLiteConstraintTranslator` weiter.
3. Implementiert `create` im Service. Prüft `pzn` vor dem Speichern und werft bei einem Duplikat die vorbereitete Konfliktausnahme.
4. Implementiert POST im Controller. Validiert den Request, ruft den Service auf und liefert 201 mit dem gespeicherten `MedicationResponse` sowie einem `Location`-Header auf die neue Ressource.
...
2. Implementiert und aktiviert in `CreateApiExampleTest` den TODO-Test `rejectsDuplicateBusinessKey` für ein doppeltes `pzn`. Legt den Konflikt über zwei Requests oder mit den Seed-Daten an. Prüft Status 409 und den Fehlerkörper.
...
GET und POST laufen durch alle Schichten. POST liefert 201 mit `Location`. Validation liefert 400, ein doppeltes `pzn` liefert 409 und eine unbekannte ID 404. Die beiden bereitgestellten Beispiele und eure beiden Tests laufen.
...
Warum reicht die vorherige `existsByPzn`-Abfrage nicht als alleiniger Schutz vor Duplikaten? Welche Fehlerdetails braucht ein Client, und welche gehören nur ins Server-Log?
