...
`Medication` ist der Domänen-Record. `MedicationJpaEntity` bildet die Tabelle für JPA ab. `SpringDataMedicationRepository` und die Infrastruktur sind vorbereitet. Der Controller, der Service und `JpaMedicationRepository` enthalten die TODOs für beide GET-Wege. `JpaLookupRepository` liest `vets`.
...
1. Implementiert `MedicationResponse.from(Medication)`.
2. Implementiert `findAll` in `JpaMedicationRepository`. Nutzt das Spring-Data-Repository, sortiert nach ID und mappt jede `MedicationJpaEntity` auf `Medication`.
3. Implementiert `findAll` im Service und danach im Controller. Der Controller mappt Domänenobjekte auf `MedicationResponse`.
...
Beide GET-Endpunkte lesen über Service, Repository-Interface und JPA-Adapter. Der Controller gibt `MedicationResponse` zurück. Eine unbekannte ID ergibt 404. Die aktivierten Repository- und API-Checkpoints laufen.
