...
Die fachliche Klasse `Medication` bleibt ein unveränderlicher Java-Record. `MedicationJpaEntity` bildet die Tabelle für JPA ab. `SpringDataMedicationRepository` erweitert `JpaRepository` und ist fertig. Offen sind `findAll` und `findById` im Adapter `JpaMedicationRepository`. Das vorbereitete `JpaLookupRepository` liest `vets`.
...
1. Implementiert `findAll` in `JpaMedicationRepository`. Nutzt das Spring-Data-Repository und mappt jede `MedicationJpaEntity` auf `Medication`.
...
4. Startet die Anwendung und prüft GET-Liste sowie GET nach ID über Swagger UI. Der Controller liefert `MedicationResponse`, keine JPA-Entity. Eine unbekannte ID ergibt 404.
...
Die Anwendung startet. Beide GET-Endpunkte liefern die Seed-Daten über das Response-DTO. ID 1 enthält `09231122`. `JpaRepositoryExerciseTest` und `OpenApiStarterTest` laufen.
...
Welche Aufgabe hat `MedicationJpaEntity`? Warum gibt der Controller trotzdem `MedicationResponse` zurück? Wo endet Spring Data und wo beginnt euer Adapter?
