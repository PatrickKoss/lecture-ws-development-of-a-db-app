...
`starter/` enthält Spring Boot, das vollständige Schema und die Seed-Daten aus B2, Response-DTO, Fehlerkörper und ein fertiges Read-Repository für `vets`. Offen sind nur die Methoden der `Medication`-Hauptressource.
...
Implementiert `findAll` und `findById` im `JdbcMedicationRepository`. GET liefert `MedicationResponse`, nie direkt die Domain-Entity. Unbekannte IDs ergeben 404.
...
Die Anwendung startet. GET-Liste und GET nach ID liefern die B2-Daten über das Response-DTO. ID 1 enthält `09231122`.
